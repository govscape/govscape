...
class ST_TextEmbeddingModel(TextEmbeddingModel):
    ...
    def encode_text_batch(self, texts, is_query=False):
        if self.model.device != self.device:
            self.model.to(self.device)
        with torch.no_grad():
            embedding = self.model.encode(texts, batch_size=GPU_BATCH_SIZE, device=self.device)
        return embedding
...
class Naive_TextEmbeddingModel(TextEmbeddingModel):
    ...
    def encode_text_batch(self, texts, is_query=False):
        return np.random.rand(len(texts), self.d)  # Return a random embedding for testing purposes
