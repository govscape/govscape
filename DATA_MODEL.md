...
* {test,dev,prod}-serving/url_blacklist.txt
...
The url_blacklist.txt file is optional. When present, it contains regular expressions (one per line, matched case-insensitively against a document's crawl_url) used to hide every digest ever crawled from a matching domain/URL, e.g. `.*\.example\.gov/.*`, without having to enumerate each digest individually. Blank lines and lines starting with `#` are ignored. Matching digests are merged into the same in-memory blacklist enforced for blacklist.txt.
