...
from govscape.data_loader import build_data_loader, RemoteDirectoryIterator

from .start_api_server import _get_arg_parser

def download_indices(args):
    # This function can be used to download the index files from S3 before starting the server.
    # It can be called separately if you want to ensure the indices are downloaded before starting the server.
    ...
    REMOTE_CHECKPOINT_PATH = os.path.join(REMOTE_DATA_DIR, "checkpoints", 'checkpoint_server.json')
    LOCAL_CHECKPOINT_PATH = os.path.join(LOCAL_DATA_DIR, "checkpoints", 'checkpoint_server.json')
    data_loader = build_data_loader(args.backend, BUCKET_NAME, local_base_dir=LOCAL_MOCK_DIR)
    for (remote_dir, local_dir) in [
        ...
        remote_iter =  RemoteDirectoryIterator(data_loader, remote_dir, REMOTE_CHECKPOINT_PATH, LOCAL_CHECKPOINT_PATH, local_dir)
        finished = False
        while not finished:
