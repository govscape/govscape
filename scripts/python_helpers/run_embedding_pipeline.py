...
def process_pdfs(pdf_files, processor, do_text_embedding, do_img_embedding, do_metadata_collection, pipeline_times,
    ...
    # PROCESS PDFS HERE 
    pdf_to_txt_img_time, text_embed_time, img_embed_time, metadata_time = processor.pdfs_to_embeddings(pdf_files,
                                                                                                    do_text_embedding,
                                                                                                    do_img_embedding,
...
if __name__ == '__main__':
    ...
    def main():
        ...
        PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../'))  # e.g. "govscape"
        LOCAL_DATA_DIR = os.path.join(PROJECT_ROOT, args.local_base_dir, 'prod')  # e.g. "govscape/data/prod"
        LOCAL_PDF_DIR = os.path.join(LOCAL_DATA_DIR, 'PDFs')  # e.g. "govscape/data/prod/PDFs"
        LOCAL_TXT_DIR = os.path.join(LOCAL_DATA_DIR, 'txt')  # e.g. "govscape/data/prod/txt"
        LOCAL_IMG_DIR = os.path.join(LOCAL_DATA_DIR, 'img')  # e.g. "govscape/data/prod/img"
        LOCAL_EMBEDDINGS_DIR = os.path.join(LOCAL_DATA_DIR, 'embeddings')  # e.g. "govscape/data/prod/embeddings"
        LOCAL_EMBEDDINGS_IMG_PG_DIR = os.path.join(LOCAL_DATA_DIR, 'embeddings_img_pg')  # e.g. "govscape/data/prod/embeddings_img_pg"
        LOCAL_METADATA_DIR = os.path.join(LOCAL_DATA_DIR, 'metadata')  # e.g. "govscape/data/prod/metadata"
        LOCAL_CHECKPOINT_PATH = os.path.join(LOCAL_DATA_DIR, 'Checkpoints', f"checkpoint_embedding_pipeline_{args.server_id}.json")  # e.g. "govscape/data/prod/Checkpoints/checkpoint_embedding_pipeline_0.json"
        ...
        LOCAL_PERF_PATH = os.path.join(LOCAL_DATA_DIR, 'Performance', f"performance_{args.server_id}.json")  # e.g. "govscape/data/prod/Performance/performance_0.json"
        ...
        # ****************************************************************************************************
        pipeline_times = {'list' : 0, 'download' : 0, 'pdf_to_txt_img_time': 0, 'text_embed_time': 0, 'img_embed_time': 0, 'metadata_time': 0, 'upload' : 0, 'pdfs_processed' : 0}  # to keep track of the time it takes for each step in the pipeline
        ...
        processor = gs.PDFsToEmbeddings(LOCAL_PDF_DIR, LOCAL_DATA_DIR, args.text_model_type, args.visual_model_type)
